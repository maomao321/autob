...
每个登录用户拥有独立的业务数据空间：配置与 API Key、订单和交易记录、AI 决策、历史行情与回测、策略运行状态及日志均互不可见。管理员角色只授予用户管理能力，不会读取或修改其他用户的业务数据。升级前已有的未分用户数据自动归最早创建的账号继续使用；服务令牌作为受信任的兼容入口，也访问这份旧数据。
...
- `POST /api/v1/backtest/downloads/update`：基于指定下载记录补齐该标的截至当前的最新 K 线。
- `POST /api/v1/backtest/runs`、`GET /api/v1/backtest/runs`、`POST /api/v1/backtest/runs/{run_id}/stop`：启动、查询或停止策略回测。
- `GET /api/v1/backtest/status`：长轮询等待下载或回测状态变更，仅在变更后返回最新任务列表。
...
2. 首次运行保留 `PAPER` 模式。打开“运行配置”页配置交易 API 和交易模式；当前界面固定使用 `binance_futures`，Stocks 选项已禁用，Futures 杠杆默认为 `1x`。
...
6. 返回“交易监控”页，添加一个或多个 Futures 标的代码；输入 `BTC`、`DRAM` 等简写会自动保存为 `BTCUSDT`、`DRAMUSDT`。大模型开关关闭时，为每个标的选择 `LONG`、`SHORT` 或 `FLAT`；开关开启时该手动值被忽略。
7. 选择标的后点击“启动所选”。每个标的有独立运行器，可分别停止。
8. 查看状态、程序持仓、持仓均价、未决订单、本次量化开仓金额和日志。大模型模式会记录今日方向、时机 `ENTER/WAIT`、置信度、决策耗时与风险。
9. 打开“交易记录”页，可按标的、开仓/平仓类型和模拟/实盘模式查询持久化成交记录。
10. 打开“AI 决策”页，可按标的和今日方向/开仓时机阶段查询历史决策，并查看最终结果、完整结构化输入、原始 API 输出、总决策耗时和模型响应时间。
11. 打开“策略回测”页，指定 Futures 标的并下载最近 180 天范围内可用的日线、5 分钟及 1 分钟 K 线；在线下载固定使用 Binance Futures，`DRAM` 等简写会自动补全为 `DRAMUSDT`。标的历史不足 180 天时以实际返回数量为准，下载完成后选择策略执行回测。
...
- 今日方向判断固定提供最近 30 根日线 OHLC（开盘、最高、最低、收盘）数据；五分钟策略产生候选突破信号后，模型根据今日日线、配置数量的最近五分钟 OHLC、结构化策略信息和信号触发数据返回 `ENTER / WAIT`。策略信息包括策略名称、方向及来源、MA7/MA25 参数与当前指标值、入场规则和预热状态；信号数据包括方向、价格、时间、突破阈值与距离、参考 K 线及各触发条件。时机 K 线数量默认为 60，可在 10～300 根之间配置。
...
- 最多 180 天的多周期分页下载固定使用 `binance_futures`；未以 `USDT` 结尾的标的会自动补全。不会以固定 K 线数量判定成败；标的上市不足 180 天时，只要各周期返回了有效数据就按实际数量完成。行情按行情源、标的、周期和开盘时间幂等写入 `orders.sqlite3`，重复下载会更新同一根 K 线。
- 下载和回测均由后端后台线程执行，关闭前端不会中断已提交任务；首次打开“策略回测”页后，前端会在当前会话中通过长轮询监听下载与回测状态，后端状态变化时立即更新任务进度。
...
- 每次点击执行回测都会直接复制提交当时策略页面中的配置，包括开仓金额、单笔/每日上限、单次持仓加仓次数、止盈止损、信号有效期和 AI 时机 K 线数量；无需先覆盖服务器的日常运行配置。该不可变副本随回测批次持久化，可在“回测明细 → 策略配置副本”查看，后续修改策略配置不会改变旧记录。均线固定为 MA7/MA25，配置副本不包含 Binance、OpenAI、DeepSeek 或 Qwen 凭据。
...
- “历史 K 线下载记录”以标的为第一列、“回测”为第二列，并显示创建时间和更新时间。完成记录可启动回测，启动时弹窗选择策略；回测运行期间同一按钮切换为停止操作。右键“更新 K 线到最新”会复用已持久化数据，仅补齐缺失区间并生成新的更新记录。
...
- 每根 K 线最多产生一次候选信号。空仓时信号用于首次开仓；已持仓时，同方向信号可在“加仓次数”限制内加仓。完全平仓后加仓计数归零，后续策略信号可开启新的持仓周期；风险退出不受加仓次数限制。
...
- 程序以实时 5 分钟 K 线的 UTC 日期划分交易日；日期变化时重置当日信号状态，但 MA7/MA25 继续使用当前时间点之前跨日连续的 25 根 K 线，乱序 K 线不会参与计算。

计算 MA7/MA25 需要当前时间点之前连续的 25 根已收盘 5 分钟 K 线。Futures 启动实时流前会通过 REST 直接获取这 25 根历史 K 线，允许跨日连续使用，不必等待启动后的实时 K 线补齐；AI 模式在收到当日日线后会同时回补配置数量的五分钟上下文。预热期间的历史信号会被丢弃，不会补下启动前的订单；进入实时流后，当前未收盘 5 分钟 K 线的最新价可以触发突破，同一根 K 线最多产生一次候选信号。若行情源返回的数据存在缺口或不足 25 根，则保持预热状态，直至形成连续样本。交易监控表按标的分别显示已实现收益与未实现收益，并提供逐行启动、停止图标按钮。
...
- 当前版本使用安全优先的单向净持仓：同一标的只能持有程序多头或程序空头之一；持仓未清零时只允许在配置次数内同向加仓，仍禁止反向开仓。Stocks 保持仅多头，Futures 支持多空。
