...
6. 打开“开平记录”页，可按标的、开仓/平仓类型和模拟/实盘模式查询持久化成交记录。
...
订单保护账本保存在后端服务器的 `orders.sqlite3`。其中保存订单标识、方向、交易日、请求金额、成交价格、成交数量、手续费、平仓收益和程序持仓，不保存 API 凭据。“开平记录”页直接查询该持久化账本，重启前后数据保持一致；页面不会展示订单号。该文件用于恢复交易次数、持仓和资金限额，不能在后端运行期间删除或修改。
...
计算 MA 交叉需要 `MA 周期 + 1` 根已收盘 5 分钟 K 线。Futures 启动实时流前会通过 REST 获取最近 6 根已收盘历史 K 线用于预热指标；历史 K 线中出现的信号会被丢弃，不会补下启动前的订单。若配置的 MA 周期大于 5，剩余样本继续由实时 K 线补齐。Stocks 仍只从启动后的实时 K 线收集样本。交易监控表按标的分别显示已实现收益与未实现收益，提供逐行启动、停止图标按钮，并支持右键启动、停止或移除标的。
