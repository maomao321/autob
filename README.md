...
交易监控表格的“手动方向”支持按股票选择 `AUTO`、`LONG`、`SHORT` 或 `FLAT`。该选择在启动股票时读取并锁定：正常取得前两个交易日的日线时仍使用自动方向；日线请求失败或有效日线不足两根时，才回退到手动方向。`AUTO` 表示不提供回退，日线不可用时保持 `UNKNOWN` 并禁止普通策略入场。手动方向会随“保存配置”写入配置文件。
...
- `DISABLED`：保持原策略，使用前两个完整交易日的收盘变化判断 `LONG / SHORT / FLAT`；日线不可用时可回退到表格中的手动方向。
...
- 大模型模式启用时使用当日模型决策作为方向；大模型模式关闭时，程序加载今日之前最近两个完整交易日的日线，前一交易日收盘价高于前二交易日为 `LONG`、低于则为 `SHORT`、相等时不交易。历史日线不足两根或加载失败时使用表格中的手动方向；手动方向为 `AUTO` 时保持 `UNKNOWN`。
...
程序收到 Binance 当日日 K 线后，会先从 Nasdaq 历史接口补全今日之前最近两个交易日的日线，用两日收盘价变化确定今日方向。计算 MA 交叉还需要 `MA 周期 + 1` 根已收盘 5 分钟 K 线，例如 MA5 需要 6 根；程序会继续从 Nasdaq 公开分时图接口加载当日分钟成交点，聚合成 5 分钟 OHLC 补全预热，再处理 Binance Stocks WebSocket 实时行情。历史数据只初始化方向和指标，不会触发补下过去的交易信号；5 分钟历史接口不可用时会降级为等待实时 K 线，日线不足或加载失败时使用启动前选定的手动方向，没有手动方向则保持 `UNKNOWN` 并禁止普通策略入场。
