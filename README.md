...
8. 打开“AI 决策”页，可按标的和今日方向/开仓时机阶段查询历史决策，并查看最终结果、完整结构化输入、原始 API 输出、总决策耗时和模型响应时间。
9. 打开“策略回测”页，指定 Futures 标的并下载最近 180 天范围内可用的日线、5 分钟及 1 分钟 K 线；标的历史不足 180 天时以实际返回数量为准，下载完成后选择策略执行回测。
...
- 收益率以配置的单笔开仓金额作为初始回测资金基数；总盈亏、胜负数和最大回撤同时保存，历史结果不会因后来修改运行配置而改变。
