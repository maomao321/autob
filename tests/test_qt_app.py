...
class QtAppWidgetTests(unittest.TestCase):
    ...
    def test_backtest_submits_current_strategy_configuration_copy(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("autoquant_frontend.pages.backtest.threading.Thread") as thread:
    ...
    def test_download_row_can_start_stop_backtest_and_update_klines(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch(
                    "autoquant_frontend.pages.backtest.QInputDialog.getItem",
                    return_value=("五分钟突破", True),
                ),
                patch("autoquant_frontend.pages.backtest.threading.Thread") as thread,
            ):
                action_button.click()
                thread.call_args.kwargs["target"]()
    ...
    def test_adding_symbol_persists_it_without_saving_other_ui_edits(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("autoquant_frontend.pages.trading.show_error") as show_error_mock:
                window._add_symbols()
    ...
    def test_contract_pool_shortcuts_open_symbol_pages(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("autoquant_frontend.pages.backtest.BacktestStatusListener"):
    ...
    def test_backtest_status_listener_tracks_visible_page_without_flashing_button(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("autoquant_frontend.pages.backtest.BacktestStatusListener") as listener_type:
            ...
            with patch("autoquant_frontend.pages.backtest.threading.Thread"):
    ...
    def test_removing_last_symbol_persists_without_saving_other_ui_edits(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch("autoquant_frontend.pages.trading.ask_yes_no", return_value=True),
                patch("autoquant_frontend.pages.trading.show_error") as show_error_mock,
            ):
    ...
    def test_remove_cancellation_keeps_symbol_and_config(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch("autoquant_frontend.pages.trading.ask_yes_no", return_value=False)
                as ask_yes_no_mock,
